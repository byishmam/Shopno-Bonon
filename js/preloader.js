...
(function () {
  ...
  /* Only the very first entry into the site (per browser session)
     should show the loading screen. Once it has played, subsequent
     page visits/navigations within the same session skip it. */
  var SESSION_KEY = "sbPreloaderShown";
  ...
  whenReady(function () {
    ...
    /* Not the first visit this session: skip the animation entirely
       and remove the markup instantly so it never flashes on screen. */
    if (alreadySeen) {
