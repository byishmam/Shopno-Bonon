...
(function () {
  ...
  function renderLists() {
    ...
    document.querySelectorAll("[data-featured-article]").forEach(function (host) {
      const item = window.ARTICLES.find(function (a) { return a.featured; }) || sorted(window.ARTICLES)[0];
  ...
  function initDetail() {
    ...
    host.innerHTML =
      '<header class="article-head"><div class="container narrow">' +
        ...
        "</div>" +
        '<div class="article-actions">' + (window.SBPdf ? window.SBPdf.buttonHTML() : "") + "</div>" +
      "</div></header>" +
