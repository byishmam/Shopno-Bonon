...
(function () {
  ...
  function buildHeader() {
    ...
    const links = S.nav.map(function (n) {
      ...
      return '<li><a href="' + n.file + '"' + active + ' data-bn="' + n.bn + '" data-en="' + n.en + '">' + n.en + "</a></li>";
    }).join("");
    ...
    panel.innerHTML =
      '<nav aria-label="Mobile"><ul>' +
        S.nav.map(function (n) {
          return '<li><a href="' + n.file + '" data-bn="' + n.bn + '" data-en="' + n.en + '">' + n.en + "</a></li>";
        }).join("") +
        '<li><a href="join-us.html" data-bn="আমাদের সাথে যুক্ত হোন" data-en="Join us">Join us</a></li>' +
      "</ul></nav>" +
  ...
  /* ---------- Magnetic / spotlight pointer effects ---------- */
  function initPointerFX() {
    const root = document.documentElement;
    if (root.classList.contains("reduce-motion") || root.classList.contains("low-power")) return;
    if (!window.matchMedia("(hover: hover) and (pointer: fine)").matches) return;

    document.addEventListener("pointermove", function (e) {
      const mag = e.target.closest(".btn, .icon-btn");
      document.querySelectorAll(".is-magnetic").forEach(function (el) {
        if (el !== mag) { el.classList.remove("is-magnetic"); el.style.removeProperty("--mx"); el.style.removeProperty("--my"); }
      });
      if (mag) {
        const r = mag.getBoundingClientRect();
        mag.classList.add("is-magnetic");
        mag.style.setProperty("--mx", ((e.clientX - r.left - r.width / 2) * 0.22).toFixed(2) + "px");
        mag.style.setProperty("--my", ((e.clientY - r.top - r.height / 2) * 0.22).toFixed(2) + "px");
      }

      const card = e.target.closest(".card, .member, .skill, .cta-panel");
      if (card) {
  ...
  function initPageTransition() {
    requestAnimationFrame(function () { document.documentElement.classList.add("page-in"); });
  }
  ...
  function boot() {
    ...
    buildHeader();
    buildFooter();
    /* FIX: the header/footer are created after language.js and theme.js have
       already run, so re-sync both control groups now that they exist. */
    L.set(L.get(), true);
    if (window.SBTheme) window.SBTheme.paint();
    L.apply(document);
    initStagger();
    initReveal();
    initParallax();
